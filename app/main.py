...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["Content-Disposition", "X-File-Name"],
)
...
@app.post("/info")
async def link_info(item: InfoRequest) -> dict:
    try:
        url = str(item.url).replace("music.youtube.com", "www.youtube.com")
...
@app.post("/download")
async def download_endpoint(item: DownloadRequest) -> CleanupFileResponse:
    try:
        ...
        url = raw_url.replace("music.youtube.com", "www.youtube.com")
